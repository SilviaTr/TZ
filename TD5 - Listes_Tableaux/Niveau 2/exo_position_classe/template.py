...
def execute_user_code(code, input_data, input_name):
    """
    Execute le code utilisateur de manière sécurisée et renvoie le résultat ou une erreur.
    """
    local_namespace = {"ordre": input_data, "nom_eleve": input_name}
    restricted_globals = {}
...
if __name__ == "__main__":
    ...
    resultat = {
        ...
        "jeux_de_donnees": [
            {
                "name_of_data_set": "Liste originale",
                "data": ["Granger", "Brown", "Finnigan", "Potter", "Thomas", "Malefoy", "Londubat", "Patil", "Weasley", "Goyle", "Crabbe"],
                "name": "Granger",
                "hidden": False,
                "validation": False,
    ...
    def input_function(ordre, nom_eleve):
        position_eleve = 0
    ...
    for jeu in resultat['jeux_de_donnees']:
        jeu['correction'] = input_function(jeu['data'], jeu['name'])
...
"""
    ...
    for jeu in resultat["jeux_de_donnees"]:
        append_log(f"Exécution du code utilisateur pour le jeu de données {jeu['name_of_data_set']}")
        result = execute_user_code(code_utilisateur, jeu["data"], jeu["name"])
        ...
        if result["success"]:
            ...
            if result["output"] == jeu["correction"]:
                jeu["validation"] = True
                append_log(f"Validation réussie pour le jeu de données {jeu['name_of_data_set']}.")
            else:
                jeu["validation"] = False
                append_log(
                    f"Validation échouée pour le jeu de données {jeu['name_of_data_set']}. Résultat attendu: {jeu['correction']}, obtenu: {result['output']}")
        else:
