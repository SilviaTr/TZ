...
def execute_user_code(code, data, func_name):
    ...
    """
    # Convertir la chaîne JSON en liste avant de la passer à la fonction
    local_namespace = {"nombre": json.loads(data)}  # Convertir 'data' en liste
    ...
    try:
        ...
        if user_function:
            resultat_utilisateur = user_function(local_namespace["nombre"])  # Passer la liste
        else:
        ...
        "bonus":[
            {
                "name": "Entrée valide",
                "validation": False,
                "message": "Vous avez validé l'entrée (ex. : un nombre positif), +1 points !",
                "impact": 1
            }],
        "console":{
            "type":None,
...
def reparo(nombre):
    # Ingrédients et leurs quantités en "louches" (1 louche = 6 petites cuillères)
    ingredients = [
        "de sève du Tesseract",
        "d'essence de Groot",
        "de nectar d'Asgard"
    ]
    quantites_converties = []

    for i in range(len(ingredients)):
        quantites_converties.append(ceil(nombre[i] * 6))
    
    return quantites_converties"""
...
for jeu in resultat["jeux_de_donnees"]:
    ...
    if result["success"]:
        ...
        expected = json.loads(jeu["correction"])  # Convertir la correction en liste
        if result["output"] == expected:
            jeu["validation"] = True
            append_log(f"Validation réussie pour le jeu de données {jeu['name']}.")
        else:
            jeu["validation"] = False
            append_log(f"Validation échouée pour le jeu de données {jeu['name']}. Résultat attendu: {expected}, obtenu: {result['output']}")
    else:
