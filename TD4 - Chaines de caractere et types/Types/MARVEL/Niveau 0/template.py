...
def execute_user_code(code, data, func_name): #code
    ...
    """
    local_namespace = {"nombre": data} #code
    restricted_globals = {}
    ...
    try:
        ...
        if user_function:
            resultat_utilisateur = user_function(local_namespace["nombre"])
        else:
...
if __name__ == "__main__":
    ...
    resultat = {
        ...
        "contraintes":[
            {
                "name": "Pas de `while` ou boucles inutiles",
                "validation": False,
                "message": "Vous avez utilisé des boucles inutiles pour cette conversion.",
                "impact": 10
            },
            {
                "name": "Pas de calculs manuels",
                "validation": False,
                "message": "Vous avez effectué des calculs manuels au lieu d'utiliser des opérateurs de calcul.",
                "impact": 20
            }],
    ...
    code_utilisateur = """def reparo(nombre):
        solution = nombre * 6
        return solution"""
    ...
    for contrainte in resultat['contraintes']:
        ...
        # Contrainte "Pas de `while` ou boucles inutiles"
        if contrainte['name'] == "Pas de `while` ou boucles inutiles":
            if "while" in code_utilisateur:
                contrainte['validation'] = False
            else:
                contrainte['validation'] = True
        
        # Contrainte "Pas de calculs manuels"
        if contrainte['name'] == "Pas de calculs manuels":
            if "*" not in code_utilisateur:  # Vérifie si le signe '*' est présent
                contrainte['validation'] = False  
            else:
                contrainte['validation'] = True  
