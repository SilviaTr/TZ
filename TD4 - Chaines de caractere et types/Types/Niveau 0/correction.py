...
if __name__ == "__main__":
    ...
    jeu_de_donnee_visible = {"name": "Jeu de données visible", "data": "0.25","impact": 3}
    ...
    dataset1 = {"name": "Jeu de test caché 1", "data": "0.75", "impact": 1}
    dataset2 = {"name": "Jeu de test caché 2", "data": "0.2", "impact": 1}
    ...
    if code_utilisateur.count("{") == 0 or code_utilisateur.count("}") == 0:
        contrainte = False
        contraintesMessages.append("Une autre solution plus élégante existe, réfléchissez aux différentes structures de données étudiées.")
        contraintesHard.append("Une autre solution plus élégante existe, réfléchissez aux différentes structures de données étudiées.")
        append_log("Contrainte non respectée: dictionnaires non utilisés")

    
    if any(f"texte[i:i+2] == '{sub}'" in code_utilisateur for sub in ['yy', 'xx', 'zz', 'ww']) and 'range(len(texte) - 1)' not in code_utilisateur: 
        contrainte = False
        contraintesMessages.append("Votre boucle ne parcourt pas l'ensemble de la chaine.")
        contraintesHard.append("Votre boucle ne parcourt pas l'ensemble de la chaine.")
        append_log("Contrainte non respectée : boucle ne parcourt pas toute la chaîne")
    ...
    if code_utilisateur.count("if") > 1:
        condition1 = True
        contraintesMessages.append("Attention une solution utilisant aucun if est possible, votre code est correct mais vous n'obtiendrez pas l'ensemble des points.")
    ...
    if code_utilisateur.count("[]") >= 1:
        condition2 = True
        contraintesMessages.append("Attention une solution n'utilisant pas de tableau possible, votre code est correct mais vous n'obtiendrez pas l'ensemble des points.")
        append_log("Points en moins: tableau utilisé")

    condition3 = False
    if code_utilisateur.count("for") > 1:
        condition3 = True
        contraintesMessages.append("Attention une solution utilisant aucun for est possible, votre code est correct mais vous n'obtiendrez pas l'ensemble des points.")
        append_log("Points en moins: trop de for")
    
    condition4 = False
    if code_utilisateur.count("while") >= 1:
        condition2 = True
        contraintesMessages.append("Attention une solution utilisant aucun while est possible, votre code est correct mais vous n'obtiendrez pas l'ensemble des points.")
