...
def execute_user_code(code, data, func_name):
    ...
    try:
        ...
        # Passage des champs ""messagee"" et "alphabet" à la fonction
        message = data.get("message", "")
        alphabet = data.get("alphabet", "")
        resultat_utilisateur = user_function(message, alphabet)
...
if __name__ == "__main__":
    ...
    resultat = {
        ...
        "jeux_de_donnees": [
            ...
            },
            {
                ...
                "data": {
                    "message": "Dwqwt cpf hqtj hqt yctf tgcejkpi vjg fgcvj jcnn",
                    "alphabet": "abcdefghijklmnopqrstuvwxyz"
                },
                "hidden": True,
                "validation": False,
            ...
            },
            {
                ...
                "data": {
                    "message": "Fgu- swcvtg eqkpu fw oqpfg, ngu jgtqu u'wpkuugpv uqwu wpg ugwng dcppkgtg : ngu Cxgpigtu.",
                    "alphabet": "abcdefghijklmnopqrstuvwxyz"
                },
                "hidden": True,
                "validation": False,
        ...
        "contraintes": [
            ...
            {
                "name": "Décalage correct",
                "validation": False,
                "message": "Le décalage dans l'alphabet n'a pas été correctement appliqué.",
                "impact": 10
            },
            {
                "name": "Gestion des caractères non alphabétiques",
                "validation": False,
                "message": "Votre code ne gère pas correctement les caractères non alphabétiques.",
                "impact": 5
            }
        ],
        "bonus": [
            {
                ...
                "validation": False,
                "message": "Votre code gère correctement les cas où les séquences sont vides ou invalides, +1points !",
                "impact": 1
            },
            {
    ...
    def fonction(message, alphabet):
        """
        Fonction pour appliquer le code César et déchiffrer le message.
        """
        alphabet_dechiffrage = ""
        for i in range(len(alphabet)):
            alphabet_dechiffrage += alphabet[(i + 2) % 26]
    ...
    for jeu in resultat['jeux_de_donnees']:
        jeu['correction'] = fonction(jeu['data']['message'], jeu['data']['alphabet'])

    append_log(f"Jeux de données: {resultat['jeux_de_donnees']}")

    # Le code utilisateur est injecté ici
    code_utilisateur = """def fonction(message, alphabet):
        alphabet_dechiffrage = ""
        for i in range(len(alphabet)):
            alphabet_dechiffrage += alphabet[(i + 2) % 26]
    ...
    for contrainte in resultat['contraintes']:
        ...
        # Contrainte de décalage correct
        if contrainte['name'] == "Décalage correct":
            if "% 26" in code_utilisateur:
                contrainte['validation'] = True
            else:
                contrainte['validation'] = False
        
        if contrainte['name'] == "Gestion des caractères non alphabétiques":
            # Vérification simple : on recherche "if message[i] in alphabet" et "else:" dans le code
            if "if message[i] in alphabet" in code_utilisateur and "else:" in code_utilisateur:
                contrainte['validation'] = True
            else:
                contrainte['validation'] = False
    ...
    for bonus in resultat['bonus']:
        ...
        if bonus['name'] == "Gestion des cas invalides":
            # Vérification si l'utilisateur gère bien les cas invalides
            pass
    ...
    for jeu in resultat["jeux_de_donnees"]:
        ...
        result = execute_user_code(code_utilisateur, jeu["data"],"fonction")
